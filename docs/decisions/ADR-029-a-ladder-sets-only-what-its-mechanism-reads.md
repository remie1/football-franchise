�# ADR-029: A ladder sets only what its mechanism reads � and what that cost CI
...
`CALIBRATION-BACKLOG.md` §22a already carries the numeric half of the method (`recordedSteps`,
...
but as a TRANSIENT state � the record of a drop in flight, not a resting place. As of this ADR
...
**2. When a ladder fails its noise margin, `games` is the only lever.** §22a forbids widening
the tolerance and §22c forbids reducing `n`; this ADR states the remaining implication
positively, so that "the gate is under-powered" resolves to an arithmetic question about sample
size rather than a negotiation. `rb-vision` (4.7ϒ) and `db-coverage` (3.85ϒ) were both fixed
this way, and in `rb-vision`'s case a tolerance widening to 0.6 would also have reached 5.0ϒ and
was rejected on this rule.

**3. Accept a +46% known-truth CI cost, and declare backlog §21a's worker-pool trigger due.**
`db-coverage` at 400 games measured its first step at 3.85ϒ over eight seed sets, against a
recorded 5.8ϒ. It went to 600 games (measured 5.8ϒ / 5.4ϒ). Because Vitest parallelises files
...
further `games` increases (`ol-passblock`, `rb-vision`, both 80 �  160) were free because they
...
- **CI owners:** the workspace test run grows ~28s. The two sanctioned remedies, when that
  becomes intolerable, are backlog §21a (`exports` map + worker threads) or §22c's split � the
...
- **Attributes pipeline:** two §5.3 kill/merge candidates are now evidenced rather than
  suspected, and both are scoped statements, not registry verdicts:
  - `touch` moves completion rate by �0�0.019 across its entire 0�95 range, non-monotonically,
    against `accuracy`'s 0.105 (100 games a rung, canonical seeds, accuracy held at 60). It is
    read only by §8.4's tight-window `QB_READ`. **On this measurement** it is a kill candidate;
  ...
  - `playRecognition` is read in the pass game by nothing at all. Its only reader is
    ...
    rung by under 0.003 yards against rung SDs of 0.06�0.11.

## Backlog amendments this lands (not yet applied � `CALIBRATION-BACKLOG.md` is outside this
dispatch's write scope)

1. **§22d's provisional list is discharged.** All five ladders have been through the full §22a
...
2. **Entry 22's "coverage saturates above 60" is FALSE.** It was drawn from an 80-game sweep.
   Re-mapped at 800 games a rung the curve is monotone at every 10-point step and 60� 95 is worth
   0.3356 yards � half the span. The real shape is a shelf at the BOTTOM (0.00283 yards per
...
3. **`dl-passrush`'s recorded step asymmetry (0.0298 / 0.0436 / 0.0435) is FALSE**, same error
   ...
   give 0.0378 / 0.0365 / 0.0361 � flat. Post-ADR-028 **both sides of §7.1 pay evenly per rating
   point from 20 to 95**, which replaces the shape-asymmetry finding ADR-028 killed.
4. **Add to §22a's shelf test:** it applies to attribute-DROP comparisons too. On the canonical
   seed list alone, dropping `playRecognition` appears to move `db-coverage`'s first step 0.3168
   �  0.3631 and its span 0.6803 �  0.7235. Across eight seed sets the effect is 0.003. One seed
   list cannot size an effect, only suggest one.
5. **Amend §22a's dispersion note:** eight seed sets is not many either. Two matched eight-set
   runs of `db-coverage` at 400 games put the same step's SD at 0.1003 and 0.0912 on means
   agreeing to 0.003 � roughly ±25% on an eight-sample SD, so a margin recorded at 4.5ϒ may
   really be 3.6ϒ.
...
**Proposal 1 � a ladder sets only what its mechanism reads.** Approved on the stated ground, which
is the right one: **gate sensitivity, not tidiness.** A ladder moving the outcome through a second
attribute's channel can stay green while the channel it is *for* is broken � entry 22a's disease
...
**Proposal 2 � when a ladder fails its noise margin, `games` is the only lever.** Approved. §22a
forbids widening the tolerance, §22c forbids reducing `n`; stating the remainder positively turns
"this gate is under-powered" from a negotiation into arithmetic. Note `rb-vision` could have
reached 5.0ϒ by widening its tolerance to 0.6 and that was rejected under this rule � which is the
rule doing work rather than describing what was already done.

**Proposal 3 � accept the +44% known-truth CI cost; §21a's worker-pool trigger is DUE but NOT
SPENT.** Approved with that split:

- **Accept the cost.** `pnpm -r test` at ~103s is not a problem, and the alternative on offer was
  a `db-coverage` gate measuring its first step at **3.85ϒ against a recorded 5.8ϒ** � the third
...
- **Do not spend the trigger yet.** The `exports`-map fix puts a build step in front of every test
  ...
  independent stages needs it � §21a's original trigger, still unmet.
- **§22c's split is the second remedy, not the first**, and remains available: fast four on every
  push, `db-coverage` on a schedule or gated to engine-touching paths. **Trimming `n` is not a
  remedy** and is not on this list.

### Left open deliberately

`db-coverage`'s `minEffect` sits at **0.25 against a measured span of 0.6429** � the effect could
halve and the gate would stay green. **Not ratcheted here**, and correctly: raising a floor inside
a re-record is a strictness change nobody asked for, and §10.1's ratchet is meant to move on
