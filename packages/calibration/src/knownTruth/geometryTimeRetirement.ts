...
/**
 * `null` means "excluded, not measured" — see `reclassifyGame`'s doc comment on the SCRAMBLE
 * boundary. Never a silent zero.
 */
function processPlay(
  buf: readonly MatchEventEnvelope[],
  finalTick: number,
  tunables: Tunables,
): DropbackReclass | null {
  // §8.8's PURSUIT CLOCK — a live pocket-status source with NO `RUSH_THREAT` publication (it is
  // typed as the weaker `ArrivalClock` precisely because it never reaches a publisher —
  // `resolve/rushThreat.ts`'s `ArrivalClock` doc comment). `activeThreats(matchups, scramble)`
  // switches ENTIRELY to the pursuit clock the tick after a `CHECK{checkKind:"scramble"}`
  // succeeds, so any POCKET_STATUS reachable from there is a claim this module cannot verify or
  // reclassify — Ruling 2 is scoped to §7.1/§7.2's THREAT model, not to §8.8's pursuit. Rather than
  // silently mis-reconstruct those ticks (which produced the identity mismatches this comment
  // replaces), the whole PLAY is excluded and counted, never guessed at.
  for (const envelope of buf) {
    if (envelope.event.type === "CHECK" && envelope.event.payload.checkKind === "scramble") return null;
  }
  ...
  for (const envelope of buf) {
    ...
    switch (event.type) {
      ...
      case "POCKET_STATUS": {
        identityChecks += 1;
        const predicted = floorFromArrival(tunables, minTtaOf(real, false));
        if (predicted !== event.payload.status) identityMismatches += 1;
        if (event.payload.status !== "CLEAN") identityPressured = true;

        const predictedCf = floorFromArrival(tunables, minTtaOf(cf, true));
...
export interface GameReclass {
  ...
  /**
   * Dropbacks EXCLUDED because a `CHECK{checkKind:"scramble"}` was observed — §8.8's pursuit clock
   * takes over pocket-status classification with no `RUSH_THREAT` publication to reconstruct it
   * from. Declared, never silently folded into either "pressured" or "not pressured".
   */
  readonly excludedForScramble: number;
}
...
export function reclassifyGame(events: readonly MatchEventEnvelope[], tunables: Tunables): GameReclass {
  ...
  const flush = (): void => {
    if (isPass && buf.length > 0) {
      const result = processPlay(buf, tick, tunables);
      if (result === null) excludedForScramble += 1;
  ...
  flush();
  return { plays: out, excludedForScramble };
}
